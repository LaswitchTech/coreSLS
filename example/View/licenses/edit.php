            ...
            <div class="flex-grow-1">
                <a href="<?= $this->getRoute() ?>?license=<?= $this->Return['license']['license'] ?>" class="nav-link px-2">
                    <h1 class="display-5 fw-lighter m-0">
                        License <span style="font-size: 0.5em;">Edit</span>
                    </h1>
                </a>
            </div>
        </div>
        ...
        <form action="?license=<?= $this->Return['license']['license'] ?>" method="post">
            <div class="form-floating mb-3">
                <select class="form-select" id="product" aria-label="product" name="product">
                    <option value="" <?= isset($_POST['product']) || $this->Return['license']['product'] ? '' : 'selected' ?>>Select Product</option>
                    <?php foreach($this->Return['products'] as $product): ?>
                        <option value="<?= $product['id'] ?>" <?= (isset($_POST['product']) && $_POST['product'] === $product['id']) || (intval($this->Return['license']['product']) === $product['id']) ? 'selected' : '' ?>><?= ucfirst($product['name']) ?><?= $product['description'] ? " - ".ucfirst($product['description']) : "" ?></option>
                    <?php endforeach; ?>
                </select>
                <label for="product">Product</label>
            </div>
            <div class="form-floating mb-3">
                <input type="text" class="form-control" id="user" name="user" placeholder="user" value="<?= isset($_POST['user']) ? $_POST['user'] : $this->Return['license']['user'] ?>">
            ...
            <div class="form-floating mb-3">
                <select class="form-select" id="term" aria-label="term" name="term">
                    ...
                    <?php foreach($this->Return['terms'] as $term): ?>
                        <option value="<?= $term ?>" <?= (isset($_POST['term']) && $_POST['term'] === $term) || ($this->Return['license']['term'] === $term) ? 'selected' : '' ?>><?= ucfirst($term) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-floating mb-3">
                <select class="form-select" id="duration" aria-label="duration" name="duration">
                    ...
                    <?php foreach($this->Return['durations'] as $duration): ?>
                        <option value="<?= $duration ?>" <?= (isset($_POST['duration']) && $_POST['duration'] === $duration) || ($this->Return['license']['duration'] === $duration) ? 'selected' : '' ?>><?= ucfirst($duration) ?></option>
                    <?php endforeach; ?>
                </select>
